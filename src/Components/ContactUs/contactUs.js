...
function ContactUs() {
  ...
  const handleChange = (e) => {
    ...
    switch (name) {
      case 'name':
        errors.name = value.trim() === '' ? 'Name is required' : (!/^[a-zA-Z ]+$/.test(value) ? 'Name is invalid' : '');
        break;
      case 'lang':
        errors.lang = value.trim() === '' ? 'Query is required' : '';
        break;
      case 'email':
        errors.email = value.trim() === '' ? 'Email Address is required' : (!/\S+@\S+\.\S+/.test(value) ? 'Invalid email address' : '');
        break;
      default:
        break;
    }
    ...
    <section style={{ marginBottom: '3rem', marginTop: '4rem' }}>
      <Breadcrumb style={{ backgroundColor: '#608da0', fontSize: '16px' }}>
        <Breadcrumb.Item active>
          <Link to="/" style={{ color: '#c8dfea', textDecoration: 'none' }}>
            Home
          </Link>
        </Breadcrumb.Item>
            ...
            <Col>
              ...
              <Form onSubmit={handleSubmit}>
                ...
                <Form.Group controlId="email" className="mb-3">
                  <Form.Label>
                    Email Address<span style={{ color: 'red', fontSize: '18px' }}>*</span>
                  </Form.Label>
                  <Form.Control
                    type="text"
                ...
                <Form.Group controlId="lang" className="mb-3">
                  <Form.Label>
                    Query to be answered<span style={{ color: 'red', fontSize: '18px' }}>*</span>
                  </Form.Label>
                  <Form.Control
                    as="textarea"
                    rows={3}
                    ...
                    value={formData.lang}
                    onChange={handleChange}
                    isInvalid={!!formErrors.lang}
                    style={{ resize: 'none' }}
                  />
                  <Form.Control.Feedback type="invalid">{formErrors.lang}</Form.Control.Feedback>
                </Form.Group>
                <Button type="submit" className="mb-3" style={{ backgroundColor: '#253f4b', border: 'none' }}>
                  Submit
                </Button>
              </Form>
            </Col>
